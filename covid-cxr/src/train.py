...
def train_experiment(cfg=None, experiment='single_train', save_weights=True, write_logs=True):
    ...
    # Set logs directory
    cur_date = datetime.datetime.now().strftime('%Y%m%d-%H%M%S')
    log_dir = rf"{cfg['PATHS']['LOGS']}training/{cur_date}" if write_logs else None
    if not os.path.exists(rf"{cfg['PATHS']['LOGS']}training/"):
        os.makedirs(rf"{cfg['PATHS']['LOGS']}training/")
    ...
    if experiment == 'hparam_search':
        log_dir = rf"{cfg['PATHS']['LOGS']}hparam_search/{cur_date}"
        random_hparam_search(cfg, data, callbacks, log_dir)
    else:
        if experiment == 'multi_train':
            base_log_dir = rf"{cfg['PATHS']['LOGS']}training/" if write_logs else None
